...
**Frontend deployed 2026-07-31** — separately from Section 15 (which only ever covered the backend VM), the frontend went live on Vercel: **https://versevault-seven.vercel.app**. Couldn't use Vercel's normal GitHub-import flow (no GitHub account access at the time), so deployed via the Vercel CLI directly from the local project instead — `vercel login` (device-code browser flow, not the GitHub OAuth option), then `vercel --yes`/`vercel --prod`. Vercel's monorepo auto-detection tried to deploy `backend/` too, as a serverless Python function — wrong, since it's a long-running Celery worker with no HTTP entrypoint; fixed by replacing the generated `vercel.json` with a single-service Vite config. `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` set as Vercel env vars by piping directly from the local `.env` file rather than retyping by hand — good thing, too: a hand-retyped copy of the anon key was caught with a byte-for-byte diff and found to actually be wrong before it shipped. **Known gap:** CLI-only deploy means no auto-redeploy-on-push; that needs the GitHub integration once account access is back.
...
### Section 10 — Failure isn't the exception, it's the design  [x] designed + implemented 2026-07-31, not live-tested
...
**Done 2026-07-31 (design + implementation; the "deliberately break it and watch" verification step was deferred by choice, given the resume-deadline push to get hosting done today — worth coming back to before calling this fully closed):** Initial instinct was "retry every exception forever" — corrected twice through direct reasoning, not just being told. First correction: traced a concrete `ValidationError` (the real `test@example.com` failure from Section 11) through a hypothetical retry loop and recognized nothing changes between attempts for a bad-input error — retrying it is pure waste. Second correction, the "shouldn't I go til success" instinct: reasoned through what already happens to a verse whose reminder failed — `last_reminded_at` stays unset, so `check_and_notify` re-flags it as due on the *next* hourly cycle — and concluded, unprompted, that this is already a system-level retry ("technically an hourly retry"), so the per-task retry doesn't need to chase eventual success on its own. Landed on: retry only `RateLimitError`/`ApplicationError` (3 attempts, exponential backoff from 5s, jitter), let `ValidationError`/`MissingRequiredFieldsError`/`MissingApiKeyError`/`InvalidApiKeyError` fail immediately. The exception classification itself came from inspecting Resend's actual SDK (`resend.exceptions.ERRORS`, mapping HTTP codes to exception classes) rather than guessing.
